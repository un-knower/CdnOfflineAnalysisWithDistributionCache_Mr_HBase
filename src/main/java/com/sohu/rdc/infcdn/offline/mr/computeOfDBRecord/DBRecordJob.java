 ...
 */
public class DBRecordJob {
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        Path output = new Path("/user/xiaoliu/dbentity/dbrecord");
...
//        conf.addResource(new Path("file:///etc/hbase/conf/hbase-site.xml"));
        ...
        job.setJarByClass(DBRecordJob.class);
        job.setMapperClass(DBRecordMapper.class);
        job.setReducerClass(DBRecordReducer.class);
        ...
        //3.表名;4.where条件
        //5.order by语句;6.列名
        String[] fields = {"minIP","maxIP", "region"};
        DBInputFormat.setInput(job, DBRecordOfIp.class, "ipseg_new", "", "", fields);
